# Here is the scaling for a large sample of Active Early-type Galaxies
import numpy as np
import random
...
# Monte Carlo sampling for luminous Halpha ETGs
def etg_sel():
    s = [1,2]
    #for i in range(n):
    while(True):
        mag = np.random.uniform(-24, -18)
        phi_lf = np.random.uniform(0, 3e-3)
        lfmag = LF(mag, mags_etg, phis_etg, alf_etg)
        if lfmag > phi_lf:
            loglha = np.random.uniform(39, 42.4)
            phi_lfha = np.random.uniform(0, 0.001)
            lfha = LFlogHa(loglha, phihas_etg1, loglha_etg, phihas_etg2, aha_etg)
            if lfha > phi_lfha:
                break
    lha = np.power(10., loglha)
    #R = Ravg
